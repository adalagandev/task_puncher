...
// Cumulative reveal caps: 5 rows show first, then "Show more" grows the window in
// widening batches (next 10 → 15 → 20, i.e. 5 → 15 → 30 → 50) before revealing the
// rest. Local UI state only, so it resets on reload (TP-023).
const REVEAL_CAPS = [5, 15, 30, 50];

// Slim, read-only queue of the active tasks beyond the focus 3 (ranks 4+). One row
// per task — rank, title, milestone progress, and the priority score — so the
// dashboard hints at what's next without the weight of full cards.
export function BacklogList({ tasks }: { tasks: Task[] }) {
      ...
      <ul className="mt-4 space-y-2">
        {rows.map((task, i) => {
          ...
          return (
            ...
            >
              {/* Rank continues the focus-3 list above, so the backlog starts at #4. */}
